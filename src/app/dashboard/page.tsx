...
import AnimatedStatCards from "@/components/AnimatedStatCardsWrapper";
...
async function getDashboardData() {
  try {
    ...
    const topDepartments = Object.entries(deptCounts).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([name, count]) => ({ name, count }));
    ...
    const assetsByMonth = Object.entries(monthCounts).sort((a, b) => a[0].localeCompare(b[0])).map(([key, count]) => {
      const [y, m] = key.split("-");
      const label = new Date(parseInt(y), parseInt(m) - 1).toLocaleDateString("en-GB", { month: "short", year: "2-digit" });
      return { month: label, count };
    });
    const recentAssets = [...typedAssets].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 8);
    const alertAssets = typedAssets.filter((a) => ["Damaged", "Under Repair", "Lost", "Stolen"].includes(a.status?.name ?? ""));
    const now = new Date();
    ...
    for (const asset of typedAssets) {
      if (asset.warranty_end_date) {
        ...
        if (d <= threshold) expiryAlerts.push({ asset, type: "Warranty", date: asset.warranty_end_date, daysRemaining: Math.ceil((d.getTime() - now.getTime()) / 86400000) });
      }
      if (asset.expected_end_of_life_date) {
        const d = new Date(asset.expected_end_of_life_date);
        if (d <= threshold) expiryAlerts.push({ asset, type: "EOL", date: asset.expected_end_of_life_date, daysRemaining: Math.ceil((d.getTime() - now.getTime()) / 86400000) });
      }
    }
    expiryAlerts.sort((a, b) => a.daysRemaining - b.daysRemaining);
    return { totalAssets: typedAssets.length, totalContacts: (contacts ?? []).length, statusCounts, topCategories, topDepartments, assetsByMonth, recentAssets, alertAssets, expiryAlerts, warrantyAlertDays, inUse: statusCounts["In Use"] ?? 0, inStorage: statusCounts["In Storage"] ?? 0 };
  } catch (err) {
...
export default async function DashboardPage() {
  ...
  if (pageError) {
    return (
      <div className="p-8">
        <h2 className="text-red-600 font-bold mb-2">Dashboard error (diagnostic)</h2>
        <pre className="text-xs bg-red-50 p-4 rounded-xl overflow-auto border border-red-200">{pageError}</pre>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-6xl">
      <div className="mb-8 fade-up">
        ...
        <h1 className="text-2xl font-semibold" style={{ letterSpacing: "-0.03em", color: "#414042" }}>Dashboard</h1>
          ...
          <div className="space-y-2">
            {data.topCategories.map(([name, count]) => (
              <div key={name} className="flex items-center gap-3 py-1">
                ...
                <span className="text-[12px] font-medium text-stone-500 font-mono bg-stone-50 px-2 py-0.5 rounded-md">{count}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white rounded-xl border border-stone-200 p-5 fade-up fade-up-3 card-lift">
          <p className="text-[12px] font-medium uppercase tracking-wider mb-4" style={{ color: "#859474" }}>Needs Attention</p>
          {data.alertAssets.length === 0 ? (
            ...
            <div className="space-y-2">
              {data.alertAssets.slice(0, 6).map((asset) => {
                ...
                return (
                  <Link key={asset.id} href={`/assets/${asset.id}`} className="flex items-center gap-2.5 py-1.5 group">
                    <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${cfg.dot}`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-[12.5px] text-stone-700 truncate group-hover:text-stone-900">#{asset.asset_code} · {asset.category?.name}</p>
              ...
              {data.alertAssets.length > 6 && <Link href="/assets" className="text-[11px] text-stone-400 hover:text-stone-600">+{data.alertAssets.length - 6} more →</Link>}
            </div>
          )}
        </div>
      </div>
      ...
      {data.expiryAlerts.length > 0 && (
        <div className="bg-white rounded-xl border border-amber-200 overflow-hidden mb-8 fade-up">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-amber-100 bg-amber-50">
            ...
            <span className="text-[11px] text-amber-500">{data.warrantyAlertDays}-day window · {data.expiryAlerts.length} item{data.expiryAlerts.length !== 1 ? "s" : ""}</span>
          </div>
          <div className="divide-y divide-stone-50">
            {data.expiryAlerts.slice(0, 8).map((alert, i) => {
              ...
              return (
                <Link key={`${alert.asset.id}-${alert.type}-${i}`} href={`/assets/${alert.asset.id}`} className="grid grid-cols-[2rem_1fr_6rem_8rem_4rem] gap-3 px-5 py-3 items-center hover:bg-stone-50 transition-colors group">
                  ...
                  <span className={`text-[11.5px] font-medium px-2 py-0.5 rounded-full w-fit ${alert.type === "Warranty" ? "bg-sky-50 text-sky-700" : "bg-purple-50 text-purple-700"}`}>{alert.type}</span>
                  <span className="text-[12px] text-stone-500">{formatDate(alert.date)}</span>
                  <span className={`text-[12px] font-medium text-right tabular-nums ${past ? "text-red-600" : soon ? "text-amber-600" : "text-stone-500"}`}>{past ? "PAST" : `${alert.daysRemaining}d`}</span>
                </Link>
              );
            })}
          </div>
      ...
      <div className="bg-white rounded-xl border border-stone-200 fade-up fade-up-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
          ...
          <Link href="/assets" className="text-[12px] flex items-center gap-1 transition-colors hover:opacity-80" style={{ color: "#C04F28" }}>View all <ArrowUpRight size={11} /></Link>
        </div>
        <div className="divide-y divide-stone-50">
          {data.recentAssets.map((asset) => {
            const cfg = getStatusConfig(asset.status?.name);
            return (
              <Link key={asset.id} href={`/assets/${asset.id}`} className="flex items-center gap-4 px-5 py-3 hover:bg-stone-50 transition-colors group">
                <span className="text-lg w-7 text-center">{getCategoryIcon(asset.category?.name)}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-[13.5px] font-medium text-stone-800 truncate">{asset.description}</p>
                  <p className="text-[11px] text-stone-400">#{asset.asset_code} · {asset.owning_department?.name ?? "—"}</p>
                </div>
