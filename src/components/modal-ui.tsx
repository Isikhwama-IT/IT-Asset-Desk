  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] overflow-y-auto px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="flex items-center justify-center min-h-full">
        {/* Panel */}
        <motion.div
          className={cn("relative bg-white rounded-2xl shadow-2xl w-full max-w-full sm:mx-auto flex flex-col", width)}
...
export function BtnPrimary({ children, loading, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { loading?: boolean }) {
  return (
    <button
      type="button"
      className="px-4 py-2 text-[13px] font-medium text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 btn-press"
...
// ─── Inline confirmation ──────────────────────────────────────────────────────
// Replaces a button with "Are you sure? Confirm / Cancel" in place.
// Usage:
...
//     loading={loading}
//     label="Unassign"           // idle label
//     confirmLabel="Yes, unassign" // confirm button label
...
export function ConfirmInline({
  ...
  const confirmCls = variant === "danger"
    ? "px-3 py-1.5 text-[12px] font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50 flex items-center gap-1.5"
    : "px-3 py-1.5 text-[12px] font-medium text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition-colors disabled:opacity-50 flex items-center gap-1.5";
  ...
  return (
    <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-2 duration-150">
      <span className="text-[12px] text-stone-500 whitespace-nowrap">Are you sure?</span>
      <button
        type="button"
        onClick={onConfirm}
        disabled={loading}
      ...
      <button
        ...
        onClick={onCancel}
        disabled={loading}
        className="px-3 py-1.5 text-[12px] font-medium text-stone-600 border border-stone-200 rounded-lg hover:bg-stone-50 transition-colors disabled:opacity-50"
      >
        Cancel
      </button>
    </div>
