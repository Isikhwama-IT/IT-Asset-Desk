...
import { AlertTriangle, ArrowUpRight, Clock, Monitor, Package, ShieldAlert, Users } from "lucide-react";
...
import { formatDate, getCategoryIcon, getStatusConfig } from "@/lib/utils";
...
const EMPTY_DATA = {
  ...
  assetsByMonth: [] as { month: string; count: number }[],
  recentAssets: [] as AssetWithRelations[],
  alertAssets: [] as AssetWithRelations[],
  expiryAlerts: [] as ExpiryAlert[],
...
async function getAssetsDashboardData() {
  try {
    ...
    const [{ data: assets }, { data: contacts }] = await Promise.all([
      supabase
        .from("assets")
        .select(`
          *,
    ...
    for (const asset of typedAssets) {
      ...
      const deptName = asset.owning_department?.name ?? "Unassigned";
    ...
    const assetsByMonth = Object.entries(monthCounts)
      ...
      .map(([key, count]) => {
        ...
        const label = new Date(parseInt(year, 10), parseInt(month, 10) - 1).toLocaleDateString("en-GB", {
          month: "short",
          year: "2-digit",
        });
    ...
    const recentAssets = [...typedAssets]
      ...
      .slice(0, 8);
    const alertAssets = typedAssets.filter((asset) =>
      ["Damaged", "Under Repair", "Lost", "Stolen"].includes(asset.status?.name ?? "")
    );
      ...
      if (asset.warranty_end_date) {
        ...
        if (d <= threshold) {
          expiryAlerts.push({
            ...
            daysRemaining: Math.ceil((d.getTime() - now.getTime()) / 86_400_000),
          });
        }
      }
      if (asset.expected_end_of_life_date) {
        ...
        if (d <= threshold) {
          expiryAlerts.push({
            asset,
            type: "EOL",
            date: asset.expected_end_of_life_date,
            daysRemaining: Math.ceil((d.getTime() - now.getTime()) / 86_400_000),
          });
        }
      }
    ...
    console.error("[AssetsDashboard] getAssetsDashboardData failed:", err);
    return EMPTY_DATA;
  }
}
  ...
  const statCards = [
    {
      ...
      icon: <Package size={15} style={{ color: "#C04F28" }} />,
    ...
    },
    {
      ...
      icon: <Users size={15} style={{ color: "#415445" }} />,
      sub: "Contacts & teams",
      accent: "#415445",
      iconBg: "#eef3e6",
    },
    {
      label: "In Use",
      value: data.inUse,
      icon: <Monitor size={15} style={{ color: "#859474" }} />,
    ...
    },
    {
      ...
      icon: <AlertTriangle size={15} style={{ color: attentionColor }} />,
  ...
  return (
    <div className="max-w-6xl space-y-8">
          ...
          {data.alertAssets.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              {data.alertAssets.length > 6 && (
                <Link href="/assets?view=list" className="text-[11px] text-stone-400 hover:text-stone-600">
      ...
      {data.expiryAlerts.length > 0 && (
        <div className="bg-white rounded-xl border border-amber-200 overflow-hidden fade-up">
          ...
          <div className="min-w-[480px] divide-y divide-stone-50">
            {data.expiryAlerts.slice(0, 8).map((alert, index) => {
              const past = alert.daysRemaining < 0;
              const soon = !past && alert.daysRemaining <= 7;
              return (
                <Link
                  key={`${alert.asset.id}-${alert.type}-${index}`}
                ...
                >
                  ...
                  <span
                    className={`text-[11.5px] font-medium px-2 py-0.5 rounded-full w-fit ${
      ...
      <div className="bg-white rounded-xl border border-stone-200 fade-up fade-up-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
          ...
          <Link
            ...
            className="text-[12px] flex items-center gap-1 transition-colors hover:opacity-80"
            style={{ color: "#C04F28" }}
          >
